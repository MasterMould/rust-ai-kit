...
start_stack() {
    ...
    # MemU is optional — warn but don't block
    local USE_MEMU=false
    if [[ -f "$MEMU_DIR/target/release/memu" ]]; then
        USE_MEMU=true
    else
        WARN "MemU not found — starting without persistent memory layer."
        WARN "To install MemU manually, see option 1 → step 6."
    fi
    ...
    if $USE_MEMU; then
        local -a MEMU_CMD=("$MEMU_DIR/target/release/memu" --api-url "http://localhost:$ENGINE_PORT")
        "${MEMU_CMD[@]}" >> "$LOG_DIR/memu.log" 2>&1 &
        local memu_pid=$!
        sleep 2
        OK "MemU started (PID: $memu_pid)"
        echo "$engine_pid $memu_pid" > "$PID_FILE"
    else
        WARN "Skipping MemU (not installed)."
